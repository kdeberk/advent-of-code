...
fn part_1(initial: &HashSet<isize>, notes: &HashSet<Vec<bool>>, n_iterations: usize) {
    ...
    for iter in 0..n_iterations {
        ...
        if ! plants_can_influence_each_other(&state) {
            ...
            println!("{}", (n_iterations - iter - 1) as isize * state.len() as isize * movement + current_sum);
            return
        }
    }

    println!("{}", state.iter().fold(0, |cur, acc| cur + acc));
}
...
fn main() {
    ...
    part_1(&initial, &notes, 20);
    part_1(&initial, &notes, 50_000_000_000);
}
