...
fn read_observations(lines: Vec<String>) -> Vec<Observation> {
    let mut lines = lines;
        ...
        match observation {
            Observation::Start { minute: _minute, guard_id } => {
                if let Some(sleep_start) = current_sleep_start {
                    if let Some(guard) = guards.get_mut(&current_guard_id.unwrap()) {
...
fn part_1(guards: &Vec<Guard>) -> u32 {
    ...
    most_asleep_guard.id * most_asleep_guard.minute_most_often_asleep()
}

fn part_2(guards: &Vec<Guard>) -> u32 {
    let mut most_asleep_guard = &guards[0];
...
pub fn solve(lines: Vec<String>) {
    let guards = read_guard_shifts(read_observations(lines));

    println!("Day 4");
    println!("Part 1: {}", part_1(&guards));
    println!("Part 2: {}", part_2(&guards));
}
