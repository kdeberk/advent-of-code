...
fn read_nanobots(lines: Vec<String>) -> Vec<Nanobot> {
    lines
        .iter()
        .map(|line| Nanobot::read_line(line))
        .collect()
}
...
pub fn solve(lines: Vec<String>) {
    let input_bots = read_nanobots(lines);

    // assert!(7 == part_1(&read_nanobots("test1")));
    // assert!(36 == part_2(&read_nanobots("test2")));

    println!("Day 23");
    println!("Part 1: {}", part_1(&input_bots));
    println!("Part 2: {}", part_2(&input_bots));
}
